...
* eP: CSS will most likely release something like the 0.1 version. We can version the new one - my proposal - as 0.2. To do that, we need to tag the current version as 0.1 so there is a reference for implementations. Later we can start calling 0.2 or so. That will avoid situations where implementations will implement something untagged.
* SC: +1
...
* eP: Both CSS and Jackson's client implement 0.1, it would be good to have stable URI for it.
* RG: If someone is targeting this implementation, should they refer to the version number or increment the year-like suffix?
* RG: I remember Aaron saying that since the names are arbitrary we just have to have unique names. Uniqueness between v1 of v2 would be adding suffix 0.1 or adding a year eg. 2022.
* eP: We can change the year for major revisions. This is for minor we can bump to 0.2 and reuse 2021.
...
* eP: Will do bikeshed and publish at GH pages. Keep in mind that it'll be the same URL for each 0. version. Should we take it slow with 0.2.
...
* eP: I'm ok with it, it adds extra overhead.
...
* SC: I would propose on open PRs and Issues and other low hanging fruits
...
* SC: Did partial review. Okay to merge but I think it needs adjustments - we generally agreed on that if I'm not mistaken any way.
* RG: I saw you proposed minor changes to the language.
* SC: I wasn't able to separate processing of the notifications.
* RG: I wanted to processing the first subscription request and processing the request for modification.
...
* RG: I would still like to add other cases which I see less essential but still useful to capture.
...
* SC: We can do this once we've ligthened our todos.
...
* SC: We have a resource which is on some server, could be any server (maybe not even solid protocol backed). We have an application that want's to watch for updates. That thing where you get channel information is still some sort of resource server, we don't expect it to be backed by a notification server. 
* RG: Notifications server needs further defining, we have some specific requirements there.
...
* RG: My concern is that we didn't want to use words client and server in protocol, we shouldn't also use them here. We were talking about requester and sender, something to consider.
...
* eP: +1. I'd like to take time to review and try to get some agreement for next meeting and then merge. I find the client problematic especially target / subscription types, the client acts as a server. The roles changes it becomes confusing. One client one server.
* SC: I didn't want to add to much about types of products specific to subscription types. I thought subscription types will describe their own classes of products.
...
* eP: The client subscribes and then maybe unsubscribes. In theory that can be a different client/software acting in the domain of subscriber. Different responsibilities and maybe isolate. Should be a single or specific/granular.. subscribing/unsubscribing.
* SC: It is design consideration, you could indefinitely divide those roles. I think it would be reasonable to say each client needs to be able to subscribe and unsubscribe. Let's be pragmatic how useful those categorisations are.
...
* SC: I thought it will be a lot of work. There was also another issue having to do with referencing AS2.0 core rather than AS2.0 Vocabulary.
...
* SC: Sometimes it is editorial call how simple do you want to communicate the minimum interop parts. If you only include parts that are essential you can get more mileage. In Jackson's PR he for example provides actor who creates the resource, which you may not want to expose in some cases.
* RG: I'm fine with that
* RG: I'm ok to merge and I can add issue to add things from my notes.
...
* eP: I think Aaron wans client to be able to rely on specific JSON-LD compaction and don't have to parse it as JSON-LD all together. Client would only treat is as JSON.
* SC: To do that we need to make a lot more. The table in that PR, covers why we remove those discrepancies.
* eP: Is to have http://www.w3.org/ns/json-ld#compacted profile be restricted to us the specific context?
* ...: So if server receives that profile in conneg it MUST use that context to compact it?
* SC: You can have expended document form and use the same vocab. 
* eP: I think we need to address https://github.com/solid/specification/issues/454 first. I believe that requirement in this PR doesn't meet Aaron's goals to use JSON-LD all together.
