...
* AC: There are ways to do multiple resources but possible messy. Inclined to say it is a feature at risk.
* SC: Should we do that for the current spec?
* AC: It is an optional feature and don't have implementations so can't say it is officially at disk. Given the target for the spec pre 1.0 it is okay to be removed for 1.0.
* SC: +1 to leave as is. Revisit.
* RG: If a client decides to watch multiple resources it needs to know that it needs the most up to date state for all those resources. What other thoughts/ideas you have for implementing and how would that reflect the spec.
* AC: I'd probably grab ETag and hash into composite or something where there is a composite structure but I don't like that structure and not sure if the state variable is best way to represent how we coordinate multiple resources. we may end up with something with a different model.
...
* SC: Should be resolved from PR 135 but vocab/JSON-LD context needs an update.
